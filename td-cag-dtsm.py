...
tf.flags.DEFINE_integer('batch_size', 3, 'How many images process at one time.')
...
tf.flags.DEFINE_string('output_dir', './outputs_td-cag-dtsm',
...
def compute_grad(x, y):
    ...
    g = tf.zeros(shape=batch_shape)
    for j in range(5):
        with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
            logits_1, end_points_1 = inception_v3.inception_v3(
                input_diversity(x), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
            logits_2, end_points_2 = inception_v4.inception_v4(
                input_diversity(x), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
            logits_3, end_points_3 = inception_resnet_v2.inception_resnet_v2(
                input_diversity(x), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        with slim.arg_scope(resnet_v2.resnet_arg_scope()):
            logits_4, end_points_4 = resnet_v2.resnet_v2_101(
                input_diversity(x), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
        ...
        n_1 = tf.gradients(cross_entropy, x)[0]
        g = g + n_1
        x = x / 2.0

    g = g / tf.constant(5.0)
    g = tf.nn.depthwise_conv2d(g, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')
...
def graph(x, y, i, x_max, x_min, xb, gb, grad):
    ...
    x_td = TD(10, x)
    g_x = compute_grad(x_td, y)

    def compute_g_xb():
        x_b = x - alpha * tf.sign(g_x)
        g_xb = compute_grad(x_b, y)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
            logits_1, end_points_1 = inception_v3.inception_v3(x_input, num_classes=FLAGS.num_classes,is_training=False)
        with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
            logits_2, end_points_2 = inception_v4.inception_v4(x_input, num_classes=FLAGS.num_classes,is_training=False)
        with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
            logits_3, end_points_3 = inception_resnet_v2.inception_resnet_v2(x_input, num_classes=FLAGS.num_classes,is_training=False)
        with slim.arg_scope(resnet_v2.resnet_arg_scope()):
            logits_4, end_points_4 = resnet_v2.resnet_v2_101(x_input, num_classes=FLAGS.num_classes, is_training=False)
        pred_labels = tf.argmax(
            tf.nn.softmax(logits_1, name='pre') + tf.nn.softmax(logits_2, name='pre') + tf.nn.softmax(logits_3,
            name='pre') + tf.nn.softmax(logits_4, name='pre'), 1)
